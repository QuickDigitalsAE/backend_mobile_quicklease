...
class UserController extends Controller
{
    ...
    {
        $this->middleware('permission:User View', ['only' => ['listUsers','editUser']]);
        $this->middleware('permission:User Add', ['only' => ['register']]);
        $this->middleware('permission:User Edit', ['only' => ['updateUser', 'userStatus']]);
    ...
    public function userStatus($id, $status)
    {
        ...
        if($status != 1 && $status != 0){
            return response()->json([
                'status' => false,
                'message' => 'The status value is incorrect. Please pass the correct value, either 1 or 0.'
            ], 200);
        }
    ...
    public function updateUser(Request $request, $id)
    {
        ...
        if ($request->has('user_enabled') && !empty($request->user_enabled)) {
